...
## Current Phase
Local remediation is complete through `BUG-0007`, and Milestone 7 non-Windows validation is green. Milestone 8 real Windows validation is IN PROGRESS; the BUG-0007 rerun reached real worker spawn and exposed `BUG-0008` in the isolated-port harness (`listenEnabled` remained false). BUG-0008 is fixed and locally verified pending checkpoint transfer. Desktop `0.1.2` remains local-only and immutable `0.1.1` remains untouched.
...
## Current Action
Checkpoint `BUG-0008` locally, transfer the exact checkpoint to `Server`, and rerun Windows zero-touch from Mainnet Node without touching the unrelated 16110/16111 service.

## Next Action
Rerun Windows zero-touch on `Server` with the BUG-0008 checkpoint; continue sequentially through any newly exposed runtime divergence, then restart/crash/relaunch and all supported Bridge modes. No real GitHub push is allowed yet.
