...
- Current engineering blocker: **BUG-0009/BUG-0010 WINDOWS REVALIDATION PENDING**. Local-first Git remains enforced; BUG-0002 through BUG-0009 are checkpointed, Milestone 7 non-Windows gates are green, and Desktop 0.1.2 is local-only. Real Windows Mainnet/Testnet10 Node lifecycle and raw-log/copy paths pass on isolated ports; BUG-0009 fixes the Bridge instance locator and BUG-0010 aligns evidence validation to the same isolated port profile. One exact Windows rerun is required. Historical PAT revocation remains separately NOT VERIFIED.
...
3. Checkpoint BUG-0010, transfer the exact local commit to `Server`, rerun Windows zero-touch to verify BUG-0009/BUG-0010 together, and continue through restart/crash/relaunch before any final publication.
