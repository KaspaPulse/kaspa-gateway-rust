...
- Current engineering blocker: **BUG-0009 WINDOWS BRIDGE REVALIDATION PENDING**. Local-first Git remains enforced; BUG-0002 through BUG-0008 are checkpointed, Milestone 7 non-Windows gates are green, and Desktop 0.1.2 is local-only. Real Windows Mainnet/Testnet10 Node lifecycle and raw-log/copy paths now pass on isolated ports; Bridge validation then exposed a stale E2E instance-port locator. The locator fix is locally green and exact Windows rerun is required. Historical PAT revocation remains separately NOT VERIFIED.
...
3. Checkpoint BUG-0009, transfer the exact local commit to `Server`, rerun Windows zero-touch through the Bridge cases, and continue through restart/crash/relaunch before any final publication.
