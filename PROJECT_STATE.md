...
- Current engineering blocker: **BUG-0008 WINDOWS REVALIDATION PENDING**. Local-first Git remains enforced; BUG-0002 through BUG-0007 are checkpointed, Milestone 7 non-Windows gates are green, and Desktop 0.1.2 is local-only. Real Windows rerun proved the BUG-0007 schema fix by reaching self-worker spawn, then exposed that the validation harness failed to enable `--listen`, causing fallback to occupied 16111. The harness fix is locally green and exact Windows rerun is required. Historical PAT revocation remains separately NOT VERIFIED.
...
3. Checkpoint BUG-0008, transfer the exact local commit to `Server`, rerun Windows zero-touch with isolated ports, and continue through restart/crash/relaunch plus supported Bridge modes before any final publication.
