...
- Current engineering blocker: **REMAINING WINDOWS RECOVERY/MODE COVERAGE**. Local-first Git remains enforced; BUG-0002 through BUG-0010 are checkpointed, Milestone 7 non-Windows gates are green, and Desktop 0.1.2 is local-only. Exact local HEAD `3414f9ca...` passed the real Windows zero-touch baseline for Mainnet/Testnet10 Node and external Bridge with zero validation errors. Restart/crash/relaunch reconciliation and in-process Bridge mode remain to be verified before release qualification. Historical PAT revocation remains separately NOT VERIFIED.
...
3. Preserve the `3414f9ca...` Windows baseline PASS and continue only with restart/crash/relaunch plus in-process Bridge mode on `Server` before any final publication.
