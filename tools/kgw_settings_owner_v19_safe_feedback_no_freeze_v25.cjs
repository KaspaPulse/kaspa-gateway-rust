...
if (!repoRoot || !reportDir || !backupDir) {
  console.error("Usage: node kgw_settings_owner_v19_safe_feedback_no_freeze_v25.cjs <repoRoot> <reportDir> <backupDir>");
  process.exit(2);
}

const OWNER = "KGW_SETTINGS_OWNER_V19";
const PATCH = "KGW_SETTINGS_OWNER_V19_SAFE_FEEDBACK_NO_FREEZE_V25";
...
function patchJs(rel, scope) {
  ...
  if (!after.includes(PATCH)) report.validationErrors.push(rel + " missing V25 patch marker.");
  if (!after.includes("const FEEDBACK_MS = 3000")) report.validationErrors.push(rel + " missing 3000ms feedback.");
  if (!after.includes("safeNoFreeze: true")) report.validationErrors.push(rel + " missing safeNoFreeze trace.");
  ...
  if (after.includes("MutationObserver")) report.validationErrors.push(rel + " still contains MutationObserver.");
  if (after.includes("requestAnimationFrame")) report.validationErrors.push(rel + " still contains requestAnimationFrame.");
  if (after.includes("setInterval(")) report.validationErrors.push(rel + " still contains setInterval.");
...
function finish(success, reason) {
  ...
  const md = [
    "# " + (success ? "SUCCESS" : "FAILED") + " - KGW Settings Owner V19 Safe Feedback No Freeze V25",
    "",
    "- Repository: `" + repoRoot + "`",
...
try {
  ...
  finish(true, "V24 freeze mechanism removed. V19 now uses safe no-freeze 3-second feedback.");
  console.log("");
  console.log("# SUCCESS");
