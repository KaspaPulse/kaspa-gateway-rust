...
if (!repoRoot || !reportDir) {
  throw new Error("Usage: node kgw_settings_buttons_single_owner_final_r14b.cjs <repoRoot> <reportDir>");
}
...
(function installKgwSettingsSingleOwnerR14B() {
  const OWNER = "KGW_SETTINGS_SINGLE_OWNER_R14B";
  ...
  function trace(root, phase, details) {
    try {
      ...
      if (window.__TAURI__ && window.__TAURI__.core && typeof window.__TAURI__.core.invoke === "function") {
        ...
        console.debug("[KGW_SETTINGS_SINGLE_OWNER_R14B]", payload);
      }
    } catch (_) {}
  }
  ...
  function describeButtons(root, network) {
    return buttons(root, network).map(function (button) {
      return {
        ...
        r14bDisabled: button.dataset ? button.dataset.kgwR14bDisabled || "" : ""
      };
    });
  }
  ...
  function setButtonsDisabled(root, network, disabled, reason) {
    buttons(root, network).forEach(function (button) {
      ...
      button.classList.toggle("kgw-settings-action-disabled-r14b", !!disabled);
      button.dataset.kgwR14bDisabled = disabled ? "true" : "false";
      button.dataset.kgwSettingsActionDisabled = disabled ? "1" : "0";
    });

    trace(root, disabled ? "r14b-buttons-disabled" : "r14b-buttons-enabled", {
      network,
      reason,
  ...
  function rememberOriginalLabel(button, action) {
    ...
    if (
      !button.dataset.kgwR14bOriginalLabel ||
      lowerCurrent === "saved" ||
      lowerCurrent === "restored" ||
      lowerCurrent.includes("تم الحفظ") ||
      lowerCurrent.includes("تمت الاستعادة")
    ) {
      button.dataset.kgwR14bOriginalLabel = current && lowerCurrent !== "saved" && lowerCurrent !== "restored"
        ? current
  ...
  function restoreOriginalLabel(button, action) {
    button.textContent = button.dataset.kgwR14bOriginalLabel || fallbackLabel(action);
  }
  ...
  function startFeedbackAfterOriginalClick(root, network, button, action) {
    ...
    setTimeout(function () {
      ...
      setButtonsDisabled(root, network, true, "r14b-action-feedback-start");
      ...
      const timer = setTimeout(function () {
        ...
        setButtonsDisabled(root, network, true, "r14b-action-feedback-end");

        trace(root, "r14b-action-feedback-complete", {
          network,
          action,
          buttons: describeButtons(root, network)
        });
      ...
      trace(root, "r14b-action-feedback-start", {
        network,
        action,
        holdMs: FEEDBACK_MS,
        buttons: describeButtons(root, network)
  ...
  function install(root) {
    if (!root || root.dataset.kgwSettingsSingleOwnerR14b === "installed") return;
    root.dataset.kgwSettingsSingleOwnerR14b = "installed";

    ["mainnet", "testnet10", "testnet12", "unknown"].forEach(function (network) {
      setButtonsDisabled(root, network, true, "r14b-initial-install");
    });
    ...
    root.addEventListener("input", function (event) {
      ...
      if (isLocked(root, network)) {
        setButtonsDisabled(root, network, true, "r14b-ignore-input-locked");
        trace(root, "r14b-setting-input-ignored-locked", {
          network,
      ...
      if (!event.isTrusted) {
        setButtonsDisabled(root, network, true, "r14b-ignore-input-programmatic");
        return;
      }

      setButtonsDisabled(root, network, false, "r14b-trusted-input");
    }, true);
    ...
    root.addEventListener("change", function (event) {
      ...
      if (isLocked(root, network)) {
        setButtonsDisabled(root, network, true, "r14b-ignore-change-locked");
        trace(root, "r14b-setting-change-ignored-locked", {
          network,
          trusted: !!event.isTrusted
        });
        return;
      ...
      if (!event.isTrusted) {
        setButtonsDisabled(root, network, true, "r14b-ignore-change-programmatic");
        return;
      }

      setButtonsDisabled(root, network, false, "r14b-trusted-change");
    }, true);
    ...
    root.addEventListener("click", function (event) {
      ...
      trace(root, "r14b-action-click-capture", {
        network,
        action,
        disabled: !!button.disabled,
      ...
      if (isLocked(root, network) || button.dataset.kgwR14bDisabled === "true") {
        event.preventDefault();
        event.stopImmediatePropagation();
        setButtonsDisabled(root, network, true, "r14b-block-click-locked");
        return;
      }
    ...
    trace(root, "r14b-owner-installed", {
      rootId: root.id || "",
  ...
  window.KGW_SETTINGS_SINGLE_OWNER_R14B = {
    install,
    setButtonsDisabled,
    buttons
  };
})();
// END_KGW_SETTINGS_SINGLE_OWNER_R14B
`;
}
...
function patchJs(file, scopeName) {
  ...
  body = body.replace(/if\s*\(\s*window\.KGW_SETTINGS_SINGLE_OWNER_R14B[\s\S]*?KGW_SETTINGS_SINGLE_OWNER_R14B\.install\(root\);\s*\}\s*/g, "");

  const injection = `{
  if (window.KGW_SETTINGS_SINGLE_OWNER_R14B && typeof window.KGW_SETTINGS_SINGLE_OWNER_R14B.install === "function") {
    window.KGW_SETTINGS_SINGLE_OWNER_R14B.install(root);
  }`;
  ...
  return {
    ...
    hasR14B: text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B"),
    hasInstallCall: text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B.install(root);"),
...
button.kgw-settings-action-disabled-r14b,
button[data-kgw-r14b-disabled="true"],
button:disabled.kgw-settings-action-disabled-r14b {
  opacity: 0.45;
  filter: grayscale(0.35);
  cursor: not-allowed;
}

button[data-kgw-r14b-disabled="false"] {
  opacity: 1;
  filter: none;
  cursor: pointer;
}
/* END_KGW_SETTINGS_SINGLE_OWNER_R14B_VISUAL */
`;
  ...
  return {
    changed: before !== text,
    hasVisual: text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B_VISUAL"),
    hasClass: text.includes("kgw-settings-action-disabled-r14b")
  };
}
...
function validateJs(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B")) {
    errors.push(label + ": missing R14B owner");
  }

  if (!text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B.install(root);")) {
    errors.push(label + ": missing installActions R14B install call");
  }
...
function validateCss(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_SINGLE_OWNER_R14B_VISUAL")) {
    errors.push(label + ": missing R14B visual marker");
  }

  if (!text.includes("kgw-settings-action-disabled-r14b")) {
    errors.push(label + ": missing R14B disabled class");
  }
...
function main() {
  ...
  writeJson("audit-before-r14b.json", beforeAudit);
  writeJson("audit-after-r14b.json", afterAudit);

  if (validationErrors.length) {
    throw new Error("R14B validation failed:\n- " + validationErrors.join("\n- "));
  }

  console.log("# R14B single settings button owner rebuild passed");
