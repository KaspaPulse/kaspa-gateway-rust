...
if (!repoRoot || !reportDir || !backupDir) {
  console.error("Usage: node kgw_settings_owner_v19_stable_feedback_label_v24.cjs <repoRoot> <reportDir> <backupDir>");
  process.exit(2);
}

const OWNER = "KGW_SETTINGS_OWNER_V19";
const PATCH = "KGW_SETTINGS_OWNER_V19_STABLE_FEEDBACK_LABEL_V24";
...
(function installKgwSettingsOwnerV19() {
  ...
  function currentLanguage() {
    ...
    try { candidates.push(document.body && document.body.getAttribute("lang")); } catch (_) {}
    ...
    try {
      for (let i = 0; i < localStorage.length; i += 1) {
        ...
        if (!key) continue;
        const lk = lower(key);
        if (lk.includes("lang") || lk.includes("locale")) {
    ...
    try {
      ...
      for (const api of apiCandidates) {
        if (!api) continue;
        candidates.push(api.language, api.lang, api.locale, api.currentLanguage, api.currentLocale);
        if (typeof api.getLanguage === "function") candidates.push(api.getLanguage());
        if (typeof api.getLocale === "function") candidates.push(api.getLocale());
      }
    } catch (_) {}
  ...
  function actionName(button) {
    const sticky = button.dataset && button.dataset.kgwSettingsOwnerV19Action;
  ...
  function rememberOriginalButtonState(button, action) {
    if (!button.dataset.kgwSettingsOwnerV19OriginalLabel) {
      button.dataset.kgwSettingsOwnerV19OriginalLabel = String(button.textContent || "").trim() || fallbackText(action);
    }

    if (!button.dataset.kgwSettingsOwnerV19Action) {
      button.dataset.kgwSettingsOwnerV19Action = action;
    }

    if (!button.__kgwSettingsOwnerV19OriginalAttributes) {
      const names = ["data-i18n", "data-i18n-key", "data-kgw-i18n", "data-label-key", "aria-label", "title"];
      const saved = {};
      names.forEach(function (name) {
        if (button.hasAttribute(name)) saved[name] = button.getAttribute(name);
      });
      button.__kgwSettingsOwnerV19OriginalAttributes = saved;
    }
  }

  function restoreOriginalAttributes(button) {
    const saved = button.__kgwSettingsOwnerV19OriginalAttributes;
    if (!saved) return;

    Object.keys(saved).forEach(function (name) {
      button.setAttribute(name, saved[name]);
    });
  }

  function suspendLabelAttributes(button) {
    const names = ["data-i18n", "data-i18n-key", "data-kgw-i18n", "data-label-key"];
    names.forEach(function (name) {
      if (button.hasAttribute(name)) {
        button.setAttribute("data-kgw-v19-suspended-" + name.replace(/^data-/, ""), button.getAttribute(name) || "");
  ...
  function restoreLabel(button) {
    restoreOriginalAttributes(button);
  ...
  function forceButtonLabel(button, label) {
  ...
  function startVisualFeedbackAfterOriginalClick(root, network, button, action) {
    window.setTimeout(function () {
      ...
      rememberOriginalButtonState(button, action);
      suspendLabelAttributes(button);

      const label = feedbackText(action);

      setDisabled(root, network, true, "feedback-clean-state");
      forceButtonLabel(button, label);
      ...
      observer.observe(button, {
        ...
        subtree: true,
        attributes: true,
        attributeFilter: ["data-i18n", "data-i18n-key", "data-kgw-i18n", "data-label-key", "aria-label", "title"]
      });

      let stopped = false;

      function keepWithAnimationFrame() {
        if (stopped) return;
        forceButtonLabel(button, label);
        const active = feedbackMap(root).get(network);
        if (active && active.button === button) {
          active.raf = window.requestAnimationFrame(keepWithAnimationFrame);
        }
      }
      ...
      feedbackMap(root).set(network, {
        ...
        interval: interval,
        observer: observer,
        raf: window.requestAnimationFrame(keepWithAnimationFrame),
        button: button,
        action: action,
        label: label
      });
...
function patchJs(rel, scope) {
  ...
  if (!after.includes(PATCH)) report.validationErrors.push(rel + " missing V24 patch marker.");
  if (!after.includes("const FEEDBACK_MS = 3000")) report.validationErrors.push(rel + " missing 3000ms feedback.");
  if (!after.includes("MutationObserver")) report.validationErrors.push(rel + " missing MutationObserver label stabilizer.");
  if (!after.includes("requestAnimationFrame")) report.validationErrors.push(rel + " missing requestAnimationFrame label stabilizer.");
  if (!after.includes("stableLabel: true")) report.validationErrors.push(rel + " missing stableLabel trace.");
  if (!after.includes("return \"تم الضبط\"")) report.validationErrors.push(rel + " missing Arabic Set feedback.");
  if (!after.includes("return \"تم الحفظ\"")) report.validationErrors.push(rel + " missing Arabic Save feedback.");
  if (!after.includes("return \"تمت الاستعادة\"")) report.validationErrors.push(rel + " missing Arabic Restore feedback.");
...
function finish(success, reason) {
  ...
  const md = [
    "# " + (success ? "SUCCESS" : "FAILED") + " - KGW Settings Owner V19 Stable Feedback Label V24",
    "",
    "- Repository: `" + repoRoot + "`",
...
try {
  ...
  finish(true, "V19 now uses stable 3-second visual feedback labels with MutationObserver/RAF stabilization.");
  console.log("");
  console.log("# SUCCESS");
