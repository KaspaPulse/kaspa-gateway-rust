...
try {
 ...
 rejected("CLI revision drift",()=>mutate(files[7],s=>s.replace(/(kaspa-rpc-core-live = .*rev = ")[a-f0-9]{40}/,(_,prefix)=>prefix+"0".repeat(40))),"cargo-rev-mismatch");
 rejected("Core revision drift",()=>mutate(files[8],s=>s.replace(/(kaspa-addresses = .*rev = ")[a-f0-9]{40}/,(_,prefix)=>prefix+"0".repeat(40))),"cargo-rev-mismatch");
 rejected("apply tool coverage drift",()=>mutate(files[9],s=>s.replace(/bridgeAliases/g,"bridgeAliasList")),"apply-script-missing-manifest-driven-coverage");
 ...
 console.log("PASS 11 source/lock/apply drift cases; no runtime processes started");
