...
if (!repoRoot || !reportDir) {
  throw new Error("Usage: node kgw_settings_buttons_canonical_owner_rebuild_r13b.cjs <repoRoot> <reportDir>");
}
...
function removeOldBlocks(text) {
  const patterns = [
    ...
    /\/\/\s*KGW_SETTINGS_CANONICAL_OWNER_R13[\s\S]*?\/\/\s*END_KGW_SETTINGS_CANONICAL_OWNER_R13\s*/g,
...
function deepCleanOldOwners(text) {
  ...
  for (const token of oldTracePhaseTokens) {
    const result = removeLinesContaining(text, token);
    text = result.text;
    if (result.changed) changed = true;
  }
...
(function installKgwSettingsCanonicalOwnerR13B() {
  const OWNER = "KGW_SETTINGS_CANONICAL_OWNER_R13B";
  ...
  function trace(root, phase, details) {
    try {
      ...
      if (window.__TAURI__ && window.__TAURI__.core && typeof window.__TAURI__.core.invoke === "function") {
        ...
        console.debug("[KGW_SETTINGS_CANONICAL_OWNER_R13B]", payload);
      }
    } catch (_) {}
  }
  ...
  function updateBaseline(root, network, reason) {
    ...
    trace(root, "r13b-baseline-updated", {
      network,
      reason,
  ...
  function describeButtons(root, network) {
    return buttons(root, network).map(function (button) {
      return {
        ...
        r13bDisabled: button.dataset ? button.dataset.kgwR13bDisabled || "" : "",
  ...
  function setDisabled(root, network, disabled, reason) {
    buttons(root, network).forEach(function (button) {
      ...
      button.classList.toggle("kgw-settings-action-disabled-r13b", !!disabled);
      button.dataset.kgwR13bDisabled = disabled ? "true" : "false";
      button.dataset.kgwSettingsActionDisabled = disabled ? "1" : "0";
    });

    trace(root, disabled ? "r13b-buttons-disabled" : "r13b-buttons-enabled", {
      network,
      reason,
  ...
  function lockKey(network) {
    return "kgwR13bLockUntil_" + network;
  }
  ...
  function rememberOriginalLabel(button) {
    if (!button.dataset.kgwR13bOriginalLabel) {
      button.dataset.kgwR13bOriginalLabel = button.textContent || "";
    }
  }

  function restoreOriginalLabel(button) {
    if (button.dataset.kgwR13bOriginalLabel) {
      button.textContent = button.dataset.kgwR13bOriginalLabel;
    }
  }
  ...
  function startActionLock(root, network, clickedButton, action) {
    ...
    setDisabled(root, network, true, "r13b-action-lock-start");
    ...
    const enforcer = setInterval(function () {
      ...
      setDisabled(root, network, true, "r13b-action-lock-enforce");
    }, 150);
    ...
    const timer = setTimeout(function () {
      ...
      updateBaseline(root, network, "r13b-action-complete-" + action);
      setDisabled(root, network, true, "r13b-action-lock-end");

      trace(root, "r13b-action-complete", {
        network,
        action,
        buttons: describeButtons(root, network)
      });
    ...
    trace(root, "r13b-action-lock-start", {
      network,
      action,
      holdMs: FEEDBACK_MS,
      buttons: describeButtons(root, network)
  ...
  function install(root) {
    if (!root || root.dataset.kgwSettingsCanonicalOwnerR13b === "installed") return;
    root.dataset.kgwSettingsCanonicalOwnerR13b = "installed";

    ["mainnet", "testnet10", "testnet12", "unknown"].forEach(function (network) {
      updateBaseline(root, network, "r13b-initial-install");
      setDisabled(root, network, true, "r13b-initial-install");
    });
    ...
    root.addEventListener("input", function (event) {
      ...
      if (isLocked(root, network)) {
        setDisabled(root, network, true, "r13b-ignore-input-locked");
        trace(root, "r13b-setting-input-ignored-locked", {
          network,
      ...
      if (!event.isTrusted) {
        setDisabled(root, network, true, "r13b-ignore-input-programmatic");
        trace(root, "r13b-setting-input-ignored-programmatic", {
          network,
          field: readField(target)
        });
        return;
      ...
      setDisabled(root, network, !dirty, dirty ? "r13b-trusted-input-dirty" : "r13b-trusted-input-back-to-baseline");
    }, true);
    ...
    root.addEventListener("change", function (event) {
      ...
      if (isLocked(root, network)) {
        setDisabled(root, network, true, "r13b-ignore-change-locked");
        trace(root, "r13b-setting-change-ignored-locked", {
          network,
          isTrusted: !!event.isTrusted,
          field: readField(target)
        });
      ...
      if (!event.isTrusted) {
        setDisabled(root, network, true, "r13b-ignore-change-programmatic");
        trace(root, "r13b-setting-change-ignored-programmatic", {
          network,
          field: readField(target)
        });
        return;
      ...
      trace(root, "r13b-setting-change", {
        network,
        field: readField(target),
        dirty,
        currentHash: snapshotHash(snapshot(root, network)),
      ...
      setDisabled(root, network, !dirty, dirty ? "r13b-trusted-change-dirty" : "r13b-trusted-change-back-to-baseline");
    }, true);
    ...
    root.addEventListener("click", function (event) {
      ...
      trace(root, "r13b-action-click-capture", {
        network,
        action,
        disabled: !!button.disabled,
      ...
      if (button.disabled || isLocked(root, network)) {
        ...
        setDisabled(root, network, true, "r13b-block-click-disabled-or-locked");

        trace(root, "r13b-action-click-blocked", {
          network,
          action,
          text: String(button.textContent || "").trim()
        });
      ...
      setTimeout(function () {
        setDisabled(root, network, true, "r13b-post-action-enforce-0ms");
      }, 0);

      setTimeout(function () {
        setDisabled(root, network, true, "r13b-post-action-enforce-300ms");
      }, 300);

      setTimeout(function () {
        setDisabled(root, network, true, "r13b-post-action-enforce-1500ms");
      }, 1500);
    }, true);

    trace(root, "r13b-canonical-owner-installed", {
      rootId: root.id || "",
  ...
  window.KGW_SETTINGS_CANONICAL_OWNER_R13B = {
    install,
    snapshot,
    buttons,
    setDisabled,
...
// END_KGW_SETTINGS_CANONICAL_OWNER_R13B
`;
}
...
function patchJs(file, scopeName) {
  ...
  body = body.replace(/if\s*\(\s*window\.KGW_SETTINGS_CANONICAL_OWNER_R13B[\s\S]*?KGW_SETTINGS_CANONICAL_OWNER_R13B\.install\(root\);\s*\}\s*/g, "");

  const injection = `{
  if (window.KGW_SETTINGS_CANONICAL_OWNER_R13B && typeof window.KGW_SETTINGS_CANONICAL_OWNER_R13B.install === "function") {
    window.KGW_SETTINGS_CANONICAL_OWNER_R13B.install(root);
  }`;
  ...
  return {
    ...
    hasR13B: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B"),
    hasInstallCall: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B.install(root);"),
    oldTokensRemaining: oldTokens.filter((token) => text.includes(token)),
    oldTracePhaseTokensRemaining: oldTracePhaseTokens.filter((token) => text.includes(token))
  };
}
...
button.kgw-settings-action-disabled-r13b,
button[data-kgw-r13b-disabled="true"],
button:disabled.kgw-settings-action-disabled-r13b {
  opacity: 0.45;
...
button[data-kgw-r13b-disabled="false"] {
  opacity: 1;
  filter: none;
  cursor: pointer;
}
/* END_KGW_SETTINGS_CANONICAL_OWNER_R13B_VISUAL */
`;
  ...
  return {
    changed: before !== text,
    hasVisual: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B_VISUAL"),
    hasClass: text.includes("kgw-settings-action-disabled-r13b")
  };
}
...
function validateJs(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B")) {
    errors.push(label + ": missing R13B owner");
  }

  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B.install(root);")) {
    errors.push(label + ": missing installActions R13B install call");
  }
  ...
  if (!text.includes("r13b-action-lock-enforce")) {
    errors.push(label + ": missing R13B lock enforcement");
  }

  const remaining = oldTokens.filter((token) => text.includes(token));
  if (remaining.length) {
    errors.push(label + ": old tokens remaining: " + remaining.join(", "));
  }

  const remainingPhase = oldTracePhaseTokens.filter((token) => text.includes(token));
  if (remainingPhase.length) {
    errors.push(label + ": old trace phase tokens remaining: " + remainingPhase.join(", "));
  }
...
function validateCss(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13B_VISUAL")) {
    errors.push(label + ": missing R13B visual marker");
  }

  if (!text.includes("kgw-settings-action-disabled-r13b")) {
    errors.push(label + ": missing R13B disabled class");
  }
...
function main() {
  const beforeAudit = {
    nodeOldTokens: oldTokens.filter((token) => read(files.nodeJs).includes(token)),
    bridgeOldTokens: oldTokens.filter((token) => read(files.bridgeJs).includes(token)),
    nodeOldTracePhaseTokens: oldTracePhaseTokens.filter((token) => read(files.nodeJs).includes(token)),
    bridgeOldTracePhaseTokens: oldTracePhaseTokens.filter((token) => read(files.bridgeJs).includes(token)),
  ...
  const afterAudit = {
    ...
    nodeCssResult,
    bridgeCssResult,
    nodeOldTokensRemaining: oldTokens.filter((token) => read(files.nodeJs).includes(token)),
    bridgeOldTokensRemaining: oldTokens.filter((token) => read(files.bridgeJs).includes(token)),
    nodeOldTracePhaseTokensRemaining: oldTracePhaseTokens.filter((token) => read(files.nodeJs).includes(token)),
    bridgeOldTracePhaseTokensRemaining: oldTracePhaseTokens.filter((token) => read(files.bridgeJs).includes(token))
  };

  writeJson("audit-before-r13b.json", beforeAudit);
  writeJson("audit-after-r13b.json", afterAudit);

  if (validationErrors.length) {
    throw new Error("R13B validation failed:\n- " + validationErrors.join("\n- "));
  }

  console.log("# R13B canonical settings button owner rebuild passed");
