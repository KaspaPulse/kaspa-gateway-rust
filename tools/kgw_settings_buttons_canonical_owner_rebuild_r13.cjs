...
if (!repoRoot || !reportDir) {
  throw new Error("Usage: node kgw_settings_buttons_canonical_owner_rebuild_r13.cjs <repoRoot> <reportDir>");
}
...
function removeOldGeneratedBlocks(text) {
  const patterns = [
...
function deepCleanOldOwners(text) {
  let changed = false;
  text = removeOldGeneratedBlocks(text);
  ...
  for (const token of oldWindowCalls) {
    const before = text;
...
(function installKgwSettingsCanonicalOwnerR13() {
  const OWNER = "KGW_SETTINGS_CANONICAL_OWNER_R13";
  ...
  function trace(root, phase, details) {
    try {
      ...
      if (window.__TAURI__ && window.__TAURI__.core && typeof window.__TAURI__.core.invoke === "function") {
        ...
        console.debug("[KGW_SETTINGS_CANONICAL_OWNER_R13]", payload);
      }
    } catch (_) {}
  }
  ...
  function snapshot(root, network) {
    const data = fields(root, network);
    const compact = data.map(function (field) {
      return {
        key: field.key,
  ...
  function hasRealDirty(root, network) {
    const current = snapshot(root, network);
    const baseline = currentBaseline(root, network);
  ...
  function describeButtons(root, network) {
    return buttons(root, network).map(function (button) {
      return {
        ...
        r13Disabled: button.dataset ? button.dataset.kgwR13Disabled || "" : "",
  ...
  function setDisabled(root, network, disabled, reason) {
    const all = buttons(root, network);

    all.forEach(function (button) {
      button.disabled = !!disabled;
      button.setAttribute("aria-disabled", disabled ? "true" : "false");
      button.classList.toggle("kgw-settings-action-disabled-r13", !!disabled);
      button.dataset.kgwR13Disabled = disabled ? "true" : "false";
      button.dataset.kgwSettingsActionDisabled = disabled ? "1" : "0";
    });

    trace(root, disabled ? "buttons-disabled" : "buttons-enabled", {
      network,
      reason,
  ...
  function isLocked(root, network) {
    const until = Number(root.dataset["kgwR13LockUntil_" + network] || "0");
    return Date.now() < until;
  }

  function rememberOriginalLabel(button) {
    if (!button.dataset.kgwR13OriginalLabel) {
      button.dataset.kgwR13OriginalLabel = button.textContent || "";
    }
  }

  function restoreOriginalLabel(button) {
    if (button.dataset.kgwR13OriginalLabel) {
      button.textContent = button.dataset.kgwR13OriginalLabel;
    }
  }

  function startFeedbackLock(root, network, clickedButton, action) {
    const until = Date.now() + FEEDBACK_MS;
    root.dataset["kgwR13LockUntil_" + network] = String(until);

    rememberOriginalLabel(clickedButton);
    clickedButton.textContent = feedbackText(action);

    setDisabled(root, network, true, "feedback-lock-start");
    ...
    const enforcer = setInterval(function () {
      ...
      setDisabled(root, network, true, "feedback-lock-enforce");
    }, 150);
    ...
    const timer = setTimeout(function () {
      ...
      root.dataset["kgwR13LockUntil_" + network] = "0";

      updateBaseline(root, network, "action-complete-" + action);
      setDisabled(root, network, true, "feedback-lock-end");

      trace(root, "feedback-complete", {
        network,
        action,
        buttons: describeButtons(root, network)
      });
    ...
    trace(root, "feedback-lock-start", {
      network,
      action,
      holdMs: FEEDBACK_MS,
      buttons: describeButtons(root, network)
  ...
  function install(root) {
    if (!root || root.dataset.kgwSettingsCanonicalOwnerR13 === "installed") return;
    root.dataset.kgwSettingsCanonicalOwnerR13 = "installed";

    ["mainnet", "testnet10", "testnet12", "unknown"].forEach(function (network) {
      updateBaseline(root, network, "initial-install");
      setDisabled(root, network, true, "initial-install");
    });
    ...
    root.addEventListener("input", function (event) {
      ...
      if (isLocked(root, network)) {
        setDisabled(root, network, true, "ignored-input-during-lock");
        trace(root, "setting-input-ignored-locked", {
          network,
      ...
      if (!event.isTrusted) {
        setDisabled(root, network, true, "ignored-programmatic-input");
        trace(root, "setting-input-ignored-programmatic", {
          network,
          field: readField(target)
        });
        return;
      ...
      setDisabled(root, network, !dirty, dirty ? "trusted-input-dirty" : "trusted-input-back-to-baseline");
    }, true);
    ...
    root.addEventListener("change", function (event) {
      ...
      if (isLocked(root, network)) {
        setDisabled(root, network, true, "ignored-change-during-lock");
        trace(root, "setting-change-ignored-locked", {
          network,
          isTrusted: !!event.isTrusted,
          field: readField(target)
        });
      ...
      if (!event.isTrusted) {
        setDisabled(root, network, true, "ignored-programmatic-change");
        trace(root, "setting-change-ignored-programmatic", {
          network,
          field: readField(target)
        });
        return;
      ...
      setDisabled(root, network, !dirty, dirty ? "trusted-change-dirty" : "trusted-change-back-to-baseline");
    }, true);
    ...
    root.addEventListener("click", function (event) {
      ...
      trace(root, "action-click-capture", {
        network,
        action,
        disabled: !!button.disabled,
      ...
      if (button.disabled || isLocked(root, network)) {
        ...
        setDisabled(root, network, true, "blocked-click-disabled-or-locked");

        trace(root, "action-click-blocked", {
          network,
          action,
          text: String(button.textContent || "").trim()
        });
      ...
      startFeedbackLock(root, network, button, action);

      setTimeout(function () {
        setDisabled(root, network, true, "post-action-enforce-0ms");
      }, 0);

      setTimeout(function () {
        setDisabled(root, network, true, "post-action-enforce-300ms");
      }, 300);

      setTimeout(function () {
        setDisabled(root, network, true, "post-action-enforce-1500ms");
      }, 1500);
    }, true);

    trace(root, "canonical-owner-installed", {
      rootId: root.id || "",
  ...
  window.KGW_SETTINGS_CANONICAL_OWNER_R13 = {
    install,
    snapshot,
    buttons,
    setDisabled,
...
// END_KGW_SETTINGS_CANONICAL_OWNER_R13
`;
}
...
function patchJs(file, scopeName) {
  ...
  body = body.replace(/if\s*\(\s*window\.KGW_SETTINGS_CANONICAL_OWNER_R13[\s\S]*?KGW_SETTINGS_CANONICAL_OWNER_R13\.install\(root\);\s*\}\s*/g, "");

  const injection = `{
  if (window.KGW_SETTINGS_CANONICAL_OWNER_R13 && typeof window.KGW_SETTINGS_CANONICAL_OWNER_R13.install === "function") {
    window.KGW_SETTINGS_CANONICAL_OWNER_R13.install(root);
  }`;
  ...
  return {
    ...
    hasR13: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13"),
    hasInstallCall: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13.install(root);"),
...
button.kgw-settings-action-disabled-r13,
button[data-kgw-r13-disabled="true"],
button:disabled.kgw-settings-action-disabled-r13 {
  opacity: 0.45;
...
button[data-kgw-r13-disabled="false"] {
  opacity: 1;
  filter: none;
  cursor: pointer;
}
/* END_KGW_SETTINGS_CANONICAL_OWNER_R13_VISUAL */
`;
  ...
  return {
    changed: before !== text,
    hasVisual: text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13_VISUAL"),
    hasClass: text.includes("kgw-settings-action-disabled-r13")
  };
}
...
function validateJs(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13")) {
    errors.push(label + ": missing R13 owner");
  }

  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13.install(root);")) {
    errors.push(label + ": missing installActions R13 install call");
  }
  ...
  if (!text.includes("feedback-lock-enforce")) {
    errors.push(label + ": missing feedback lock enforcement");
  }
...
function validateCss(file, label) {
  ...
  if (!text.includes("KGW_SETTINGS_CANONICAL_OWNER_R13_VISUAL")) {
    errors.push(label + ": missing R13 visual marker");
  }

  if (!text.includes("kgw-settings-action-disabled-r13")) {
    errors.push(label + ": missing R13 disabled class");
  }
...
function main() {
  ...
  writeJson("audit-before-r13.json", beforeAudit);
  writeJson("audit-after-r13.json", afterAudit);

  if (validationErrors.length) {
    throw new Error("R13 validation failed:\n- " + validationErrors.join("\n- "));
  }

  console.log("# R13 canonical settings button owner rebuild passed");
