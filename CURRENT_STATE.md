...
- Current HEAD: **VERIFY DYNAMICALLY**; latest committed local checkpoint is `583ac6204609f6eb80504fe797eb828a2ff2ea53` (Windows E2E isolated-port profile).
...
- Working tree: **DIRTY** intentionally with locally verified `BUG-0007` serde contract fix, regression coverage, and state evidence pending checkpoint commit; classify dynamically on resume.
...
- Local checkpoints already verified on `local`: `e2b00ef` task open, `147b57e` BUG-0002, `dc41641` BUG-0003, `1f1abe6` BUG-0004, `b948849` BUG-0005, `2e1c545` close/relaunch protection, `5242804` strict-Clippy cleanup, `8bdaff4` BUG-0006, `4cbf127` Desktop 0.1.2 candidate metadata, `583ac62` Windows E2E isolated-port profile.
...
## NEXT ACTION
Checkpoint `BUG-0007`, transfer the exact local branch checkpoint to `Server`, and rerun zero-touch with isolated ports; continue from the first subsequent Windows divergence only.
